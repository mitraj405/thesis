...
contract GovernorZama {
    ...
    function propose(
        ...
        newProposal.forVotes = FHEUInt.wrap(0);
        newProposal.againstVotes = FHEUInt.wrap(0);
    ...
    function isDefeated(Proposal storage proposal) private view returns (bool) {
        try
            Common.requireCt(
                FHEOps.lte(proposal.forVotes, proposal.againstVotes)
            )
        {
            return true;
        } catch (bytes memory) {
            FHEUInt encryptedQuorumVotes = Ciphertext.from(
                uint8(quorumVotes())
            );
            try
                Common.requireCt(
                    FHEOps.lt(proposal.forVotes, encryptedQuorumVotes)
                )
            {
                return true;
            } catch (bytes memory) {
        ...
        // if (FHEOps.lte(proposal.forVotes, proposal.againstVotes)) ||
        //     FHEOps.lt(proposal.forVotes, quorumVotes()))
    ...
    function castVote(uint proposalId, bytes calldata support) public {
        return castVote(proposalId, Ciphertext.verify(support));
    }

    function castVote(uint proposalId, FHEUInt support) public {
        return _castVote(msg.sender, proposalId, support);
    }

    function castVoteBySig(
        uint proposalId,
        bytes calldata support,
        uint8 v,
        bytes32 r,
        bytes32 s
    ) public {
        return castVoteBySig(proposalId, Ciphertext.verify(support), v, r, s);
    }
    ...
    function _castVote(
        ...
        FHEUInt votes = comp.getPriorVotes(voter, proposal.startBlock);
        ...
        FHEUInt ct_one = Ciphertext.from(uint8(1));

        proposal.forVotes = FHEOps.add(
            proposal.forVotes,
            FHEOps.mul(votes, support)
        );

        proposal.forVotes = FHEOps.add(
            proposal.forVotes,
            FHEOps.mul(votes, FHEOps.sub(ct_one, support))
        );
